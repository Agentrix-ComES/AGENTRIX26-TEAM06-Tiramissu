...
def _determine_poi_type(tags: dict) -> str:
    ...
    if tags.get("historic") in ["archaeological_site", "monument", "ruins"]: return "History"
    if tags.get("amenity") == "place_of_worship": return "Temple/Shrine"
    if tags.get("natural") in ["beach", "coastline"]: return "Beach"
    if tags.get("natural") in ["water", "peak", "wood", "forest"]: return "Nature"
    if tags.get("tourism") == "museum": return "Museum"
    if tags.get("amenity") in ["restaurant", "cafe"]: return "Food"
...
def _extract_poi_from_osm_element(element: dict) -> dict | None:
    ...
    cost_estimate = 0
    if fee in ["yes", "true", "1"]: cost_estimate = 1500
    elif fee in ["no", "false", "0", "free"]: cost_estimate = 0
...
def _generate_fallback_pois(lat: float, lon: float) -> str:
    ...
    # Filter to nearest 2
    for p in fallbacks: p["dist"] = _haversine_distance(lat, lon, p["lat"], p["lon"])
    fallbacks.sort(key=lambda x: x["dist"])
    return json.dumps(fallbacks[:2])

@tool
def search_tourist_pois(lat: float, lon: float, radius: int, interests: str, budget: int, time_available: int) -> str:
    ...
    Returns:
        JSON string of matched POIs with coordinates and estimated costs.
    """
    radius = min(radius, 10000)
    interests_lower = interests.lower()
    
    query_filters = []
    if "temple" in interests_lower or "religion" in interests_lower:
